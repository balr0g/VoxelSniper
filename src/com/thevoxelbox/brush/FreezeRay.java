...
/**
 *
 * @author Gavjenks (derived from Piotr's ball replace brush)
 */
...
public class FreezeRay extends Brush {
    ...
    public void parameters(String[] par, vSniper v) {
        if (par[1].equalsIgnoreCase("info")) {
            v.p.sendMessage(ChatColor.LIGHT_PURPLE + "Freeze Ray Parameters:");
            v.p.sendMessage(ChatColor.BLUE + "h[number] (ex:  h20) Maximum crystal height");
            v.p.sendMessage(ChatColor.GOLD + "s[number] (ex:   s0.9) Sets the hardness of the sigmoid curve.  must be between 0 and 1.  Closer to zero = you will have sudden, big differences in crystal heights near the middle.");
            v.p.sendMessage(ChatColor.DARK_GREEN + "f[number] (ex:   f200) 1/f = likelihood of a crystal growing out of any given block.");
        ...
        for (int x = 1; x < par.length; x++) {
            if (par[x].startsWith("h")) {
                ...
                v.p.sendMessage(ChatColor.BLUE + "Max height of crystals set to " + height);
            } else if (par[x].startsWith("f")) {
                frequency = Double.parseDouble(par[x].substring(0));
                v.p.sendMessage(ChatColor.DARK_GREEN + "1/f frequency of crystals set to " + frequency);
                ...
                v.p.sendMessage(ChatColor.GOLD + "Sigmoid set to " + sigmoid);
            } else {
        ...
        for (int z = bsize; z >= 0; z--) {
            for (int x = bsize; x >= 0; x--) {
                for (int y = bsize; y >= 0; y--) {
                    if ((Math.pow(x, 2) + Math.pow(y, 2) + Math.pow(z, 2)) <= bpow) {
                        for (int i = 1; i < 9; i++) { //this just avoids me copying and pasting all those huge bits of code below 8 times.  More compact to look at.
                            ...
                            if (octant == 10 || octant == 11) {
                                h.put(s.getBlockAt(octX, octY, octZ));
                            ...
                            //                    v.p.sendMessage(ChatColor.RED + "Matrix out of bounds. [debug]");
                            //                }
                            //          }
                            //        }
                            //    }
                                ...
                                //top and bottom
                                    h.put(s.getBlockAt(octX, octY+r, octZ));
                                    setBlockIdAt(79, octX, octY+r, octZ);
                                    h.put(s.getBlockAt(octX, octY, octZ));
                                    setBlockIdAt(79, octX, octY, octZ);
                                    //column
                                for (int f = 1;f<rr;f++){
                                    h.put(s.getBlockAt(octX, octY+f, octZ));
                                    setBlockIdAt(79, octX, octY+f, octZ);
                                    h.put(s.getBlockAt(octX + 1, octY+f, octZ));
                                    setBlockIdAt(79, octX + 1, octY+f, octZ);
                                    h.put(s.getBlockAt(octX - 1, octY+f, octZ));
                                    setBlockIdAt(79, octX - 1, octY+f, octZ);
                                    h.put(s.getBlockAt(octX, octY+f, octZ + 1));
                    ...
                    }//end if for whether it's in the brush or not.
                }//Y
            }//X
