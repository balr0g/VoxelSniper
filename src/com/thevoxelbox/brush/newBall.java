...
public class newBall extends Brush {
    ...
    protected void arrow(vSniper v) {
        ...
        for (int i = bsize; i >= -bsize; i--) {
            performArrow(s.getBlockAt(bx, by + i, bz));
        }
        for (int i = bsize; i >= 1; i--) {
            performArrow(s.getBlockAt(bx + i, by, bz));
            performArrow(s.getBlockAt(bx - i, by, bz));
            performArrow(s.getBlockAt(bx, by, bz + i));
            performArrow(s.getBlockAt(bx, by, bz - i));
        }
            ...
            for (int x = bsize; x >= 1; x--) {
                ...
                for (int z = bsize; z >= 1; z--) {
                    ...
                    if ((xpow + Math.pow(y, 2) + zpow) <= bpow) {
                        performArrow(s.getBlockAt(bx + x, by + z, bz + y));
                        performArrow(s.getBlockAt(bx + x, by + z, bz - y));
                        performArrow(s.getBlockAt(bx - x, by + z, bz + y));
                        performArrow(s.getBlockAt(bx - x, by + z, bz - y));
                        performArrow(s.getBlockAt(bx + x, by - z, bz + y));
    ...
    protected void powder(vSniper v) {
        ...
        double bpow = Math.pow(bsize + trueCircle, 2);
        for (int i = bsize; i >= -bsize; i--) {
            performPowder(s.getBlockAt(bx, by + i, bz));
        }
        for (int i = bsize; i >= 1; i--) {
            performPowder(s.getBlockAt(bx + i, by, bz));
            performPowder(s.getBlockAt(bx - i, by, bz));
            performPowder(s.getBlockAt(bx, by, bz + i));
            performPowder(s.getBlockAt(bx, by, bz - i));
        }
        for (int z = bsize; z >= 1; z--) {
            double zpow = Math.pow(z, 2);
            for (int x = bsize; x >= 1; x--) {
                ...
                for (int y = bsize; y >= 1; y--) {
                    if ((xpow + Math.pow(y, 2) + zpow) <= bpow) {
                        performPowder(s.getBlockAt(bx + x, by + z, bz + y));
                        performPowder(s.getBlockAt(bx + x, by + z, bz - y));
                        performPowder(s.getBlockAt(bx - x, by + z, bz + y));
                        performPowder(s.getBlockAt(bx - x, by + z, bz - y));
                        performPowder(s.getBlockAt(bx + x, by - z, bz + y));
