...
public class Vegetation extends Brush {
    ...
    public void parameters(String[] par, vSniper v) {
        ...
        if (par[1].equalsIgnoreCase("info") || par[1].equalsIgnoreCase("help")) {
            v.p.sendMessage(ChatColor.GOLD + "Vegetation brush parameters");
            v.p.sendMessage(ChatColor.GREEN + "b[number] (ex:  b23) Sets your sniper brush size.");
            v.p.sendMessage(ChatColor.GREEN + "ld[number] (ex: ld7.5) Sets the % dencity of large foliage.");
            v.p.sendMessage(ChatColor.GREEN + "sd[number] (ex: sd33.33) Sets the % dencity of small foliage.");
            v.p.sendMessage(ChatColor.GREEN + "fl[number] (ex: sd10.4) Sets the chance of flowers rather than grass.");
            v.p.sendMessage(ChatColor.GREEN + "t[number][tree type],[number][tree type] (ex: t:%66.66tree,33.33%bigtree) ");
            v.p.sendMessage(ChatColor.GREEN + "      Sets the type of tree to be generated, and the chance of them. ");
            v.p.sendMessage(ChatColor.GREEN + "      Use t:[treeType] for one kind of tree. ");
            v.p.sendMessage(ChatColor.GREEN + "      Can be: bigTree, birch, redwood, tallredwood, or tree.");
            v.p.sendMessage(ChatColor.GOLD + "For user-friendly pre-sets, type /b vg info2.");

            return;
        }
        if (par[1].equalsIgnoreCase("info2")) {
            v.p.sendMessage(ChatColor.GOLD + "User-friendly Preset Options.  These are for the arrow.  Powder will do reverse for the first two (for fast switching):");
            v.p.sendMessage(ChatColor.GREEN + "/b vg meadow -- for sparce, grass-filled areas.");
            v.p.sendMessage(ChatColor.GREEN + "/b vg forest -- for medium dencity forests.");
            v.p.sendMessage(ChatColor.GREEN + "/b vg [desert|thin] -- for desert areas or very thin areas.");
            v.p.sendMessage(ChatColor.GREEN + "/b vg flower -- generates a flower patch.");
            v.p.sendMessage(ChatColor.GREEN + "/b vg grass -- generates a patch of grass.");
            v.p.sendMessage(ChatColor.GREEN + "/b vg patch -- generates a patch of grass and flowers.");
        ...
        for (int x = 1; x < par.length; x++) {
            try {
                ...
                if (par[x].startsWith("meadow")) {
                    ...
                    v.p.sendMessage(ChatColor.AQUA + "Meadow mode. (/b vg b16 fl10 ld5 sd40 t75%tree,25%bigtree)");
                    continue;
                    ...
                    v.p.sendMessage(ChatColor.AQUA + "Forest mode. (/b vg b16 fl10 ld5 sd40 t70%tree,20%bigtree,10%birch)");
                    continue;
                    ...
                    smDencity = 10f;
                    trees = new EnumMap<TreeType, Float>(TreeType.class);
                    v.p.sendMessage(ChatColor.AQUA + "Derert/thin mode. (/b vg b16 fl0 ld5 sd10 t:normal)");
                    continue;
                    ...
                    smDencity = 70f;
                    trees = new EnumMap<TreeType, Float>(TreeType.class);
                    v.p.sendMessage(ChatColor.AQUA + "Flower patch mode. (/b vg b5 fl100 ld0 sd70)");
                    continue;
                    ...
                    smDencity = 70f;
                    trees = new EnumMap<TreeType, Float>(TreeType.class);
                    v.p.sendMessage(ChatColor.AQUA + "Grass patch mode. (/b vg b5 fl0 ld0 sd70)");
                    continue;
                    ...
                    smDencity = 70f;
                    trees = new EnumMap<TreeType, Float>(TreeType.class);
                    v.p.sendMessage(ChatColor.AQUA + "Patch mode. (/b vg b5 fl25 ld0 sd70)");
                    continue;
                } //Peramiters
                else if (par[x].startsWith("ld")) {
                    lgDencity = Float.parseFloat(par[x].replace("ld", ""));
                    v.p.sendMessage(ChatColor.BLUE + "Dencity of large vegetation set to " + lgDencity);
                    continue;
                } else if (par[x].startsWith("sd")) {
                    smDencity = Float.parseFloat(par[x].replace("sd", ""));
                    v.p.sendMessage(ChatColor.BLUE + "Dencity of small vegetation set to " + smDencity);
                    continue;
                } else if (par[x].startsWith("fl")) {
                    smDencity = Float.parseFloat(par[x].replace("fl", ""));
                    v.p.sendMessage(ChatColor.BLUE + "Chance of flowers set to " + flowerProb);
                    continue;
                } else if (par[x].startsWith("b")) {
                    brushSize = Integer.parseInt(par[x].replace("b", ""));
                    v.p.sendMessage(ChatColor.BLUE + "Size of brush set to " + brushSize);
                    continue;
                    ...
                    v.p.sendMessage(ChatColor.BLUE + "Tree type set to: " + arg);
                    continue;
                }
                continue;
            } catch (Exception e) {
    ...
    private void generate(vSniper v) {
                ...
                if (rand.nextFloat()<lgDencity){
                    ...
                    if (top.getType()==Material.GRASS){
                        ...
                        for (TreeType t:trees.keySet()){
                         ...
                         s.generateTree(new Location(s, x,s.getHighestBlockYAt(x, z)+1,z), type);
    ...
    private void degenerate(vSniper v) {
