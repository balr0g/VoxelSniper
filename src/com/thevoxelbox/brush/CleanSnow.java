            ...
            for (int x = (bsize + 1) * 2; x >= 0; x--) {
                ...
                for (int z = (bsize + 1) * 2; z >= 0; z--) {
                    if ((xpow + Math.pow(z-bsize, 2) + ypow) <= bpow) {
                        if ((s.getBlockAt(bx + x - bsize, by + z - bsize, bz + y - bsize).getType() == Material.SNOW) && ((s.getBlockAt(bx + x - bsize, by + z - bsize - 1, bz + y - bsize).getType() == Material.SNOW) || (s.getBlockAt(bx + x - bsize, by + z - bsize - 1, bz + y - bsize).getType() == Material.AIR))) {
