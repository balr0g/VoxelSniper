...
public class SplatterDisc extends PerformBrush {
    ...
    public void parameters(String[] par, vSniper v) {
        if (par[1].equalsIgnoreCase("info")) {
            v.p.sendMessage(ChatColor.GOLD + "Splatter Disc brush Parameters:");
            v.p.sendMessage(ChatColor.AQUA + "/b sd s[int] -- set a seed percentage (1-9999). 100 = 1% Default is 1000");
            v.p.sendMessage(ChatColor.AQUA + "/b sd g[int] -- set a growth percentage (1-9999).  Default is 1000");
            v.p.sendMessage(ChatColor.AQUA + "/b sd r[int] -- set a recursion (1-10).  Default is 3");
            return;
        }
    ...
    public void splatterdisc(vSniper v) {
        if (seedpercent < 1 || seedpercent > 9999) {
