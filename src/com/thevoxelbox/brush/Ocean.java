...
//This would allow for higher or lower oceans. It's also more useful on maps
...
public class Ocean extends Brush {
    ...
    protected void oceanator(vSniper v) {
        ...
        if (tb.getX() >= 0 && tb.getZ() >= 0) {
            ...
            tb = s.getBlockAt(sx + 8, tb.getY(), sz + 8);
    ...
    protected Block setX(Block bl, int bx) {
        return s.getBlockAt(bx, bl.getY(), bl.getZ());
    }

    protected Block setZ(Block bl, int bz) {
        return s.getBlockAt(bl.getX(), bl.getY(), bz);
    }
