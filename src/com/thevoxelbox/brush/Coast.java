...
public class Coast extends Brush {
    ...
    public void coast(vSniper v) {
        ...
        byte[][] mask = new byte[maxX - minX + 4][maxZ - minZ + 4]; //+1 just so that both ends of the range are inclusive.  +2 more to give a border around the mask which is needed later, and +1 more to make it so I don't have to remember to type in annoying -1's all the time to access arrays.
            ...
            for(int y = 128; y > 0;y--){
                for(int z = 0; z < maxZ-minZ+4; z++){
                    ...
                    if (memory[x][z][5]==0){ //if column hasn't been memorized already
                        if (getBlockIdAt(x, y-1, z) != 0) { //if not a floating block (like one of Notch's pools)
                            switch (getBlockIdAt(x, y, z)) {
                                ...
                                default:
                                    h.put(s.getBlockAt(x, y, z)); //destroy anything above the top four layers that isn't natural or is floating by itself
                                    setBlockIdAt(0, x, y, z);
                                    break;
                            }
                        }
        ...
        //Then accrete two steps' worth of pixels to the mask from the outside, and then erode one step's worth.  Should result in a smooth-ish next layer.  If not, figure out a better smoothing method.
