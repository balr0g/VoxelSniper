...
/**
 *
 * @author Gavjenks (from Gavjenks & Piotr's 2D brush)
 */
...
public class Rot3D extends Brush {
    ...
    private void rotate(vSniper v) {
        ...
        //  --> Why not utilize Sniper's new oportunities and have arrow rotate all 3, powder rotate x, goldsisc y, otherdisc z. Or something like that. Or we could just use arrow and powder and just differenciate between left and right click that gis 4 different situations
            ...
            for (int z = 0; z < snap.length; z++) {
                ...
                double zpow = Math.pow(zz, 2);
                    ...
                    if (xpow + zpow + Math.pow(yy, 2) <= bpow) {
                        h.put(s.getBlockAt(bx + xx, by + yy, bz + zz)); //just store whole sphere in undo, too complicated otherwise, since this brush both adds and remos things unpredictably.
