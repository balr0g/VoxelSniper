...
public class Extrude extends Brush {
                ...
                if ((xpow + Math.pow(y, 2)) <= bpow) {
                    if (awto) {
                        for (int i = 0; i <= level - 1; i++) {
                            h = perform(s.getBlockAt(bx + i, by + x, bz + y), s.getBlockAt(bx + i + 1, by + x, bz + y), v, h);
                            h = perform(s.getBlockAt(bx + i, by + x, bz - y), s.getBlockAt(bx + i + 1, by + x, bz - y), v, h);
                            h = perform(s.getBlockAt(bx + i, by - x, bz + y), s.getBlockAt(bx + i + 1, by - x, bz + y), v, h);
                            h = perform(s.getBlockAt(bx + i, by - x, bz - y), s.getBlockAt(bx + i + 1, by - x, bz - y), v, h);
                        }
        ...
        for (int x = bsize; x >= 0; x--) {
            ...
            for (int y = bsize; y >= 0; y--) {
                if ((xpow + Math.pow(y, 2)) <= bpow) {
                   if (awto) {
                        for (int i = 0; i <= level - 1; i++) {     
                            h = perform(s.getBlockAt(bx + x, by + y, bz + i), s.getBlockAt(bx + x, by + y, bz + i + 1), v, h);
                            h = perform(s.getBlockAt(bx + x, by - y, bz + i), s.getBlockAt(bx + x, by - y, bz + i + 1), v, h);
                            h = perform(s.getBlockAt(bx - x, by + y, bz + i), s.getBlockAt(bx - x, by + y, bz + i + 1), v, h);
                            h = perform(s.getBlockAt(bx - x, by - y, bz + i), s.getBlockAt(bx - x, by - y, bz + i + 1), v, h);
                        }
        ...
        for (int x = bsize; x >= 0; x--) {
            ...
            for (int y = bsize; y >= 0; y--) {
                if ((xpow + Math.pow(y, 2)) <= bpow) {
                    if (awto) {
                        for (int i = 0; i <= level - 1; i++) {
                            h = perform(s.getBlockAt(bx + x, by + i, bz + y), s.getBlockAt(bx + x, by + i + 1, bz + y), v, h);
                            h = perform(s.getBlockAt(bx + x, by + i, bz - y), s.getBlockAt(bx + x, by + i + 1, bz - y), v, h);
                            h = perform(s.getBlockAt(bx - x, by + i, bz + y), s.getBlockAt(bx - x, by + i + 1, bz + y), v, h);
                            h = perform(s.getBlockAt(bx - x, by + i, bz - y), s.getBlockAt(bx - x, by + i + 1, bz - y), v, h);
                        }
    ...
    private vUndo perform(Block b1, Block b2, vSniper v, vUndo h) {
        ...
        if (v.voxelList.contains(getBlockIdAt(b1.getX(), b1.getY(), b1.getZ()))) {
            ...
            s.getBlockAt(b2.getX(), b2.getY(), b2.getZ()).setData(s.getBlockAt(b1.getX(), b1.getY(), b1.getZ()).getData());
        }
