...
public class ForceBrush extends Brush {
    ...
    public void Attract(vSniper v) {
        ...
        if (passCorrect) {
            ...
            if (RADIUS != -3) {
                List nearbyEnts = s.getLivingEntities();
                Location loc = s.getBlockAt(bx, by, bz).getLocation(); //location of snipe
                try {
            ...
            try {
                for (int i = 0; i < nearbyEnts.size(); i++) {
                    ...
                    if (xdist2 + ydist2 + zdist2 <= Math.pow(RADIUS, 2)) {
                        ...
                        Location toTel = new Location(s, bx + xdist, by + ydist, bz + zdist);
