...
public class BlendVoxelDisc extends Brush {
    ...
    public void parameters(String[] par, vSniper v) {
        if (par[1].equalsIgnoreCase("info")) {
            v.p.sendMessage(ChatColor.GOLD + "Blend Voxel Disc Parameters:");
            //v.p.sendMessage(ChatColor.AQUA + "/b bvd air -- toggle include or exclude (default) air");
            v.p.sendMessage(ChatColor.AQUA + "/b bvd water -- toggle include or exclude (default) water");
            return;
        }
       ...
       //Log current materials into oldmats
        ...
        for (int x = 0; x <= 2 * bsize; x++) {
            for (int z = 0; z <= 2 * bsize; z++) {
                ...
                //Make sure there's not a tie for most common
            ...
            for (int z = 2 * bsize; z >= 0; z--) {
                ...
                if (!(ablendmode.equalsIgnoreCase("exclude") && newmats[x][z] == 0)&& !(wblendmode.equalsIgnoreCase("exclude") && (newmats[x][z] == 8||newmats[x][z]==9))) {
                    if (getBlockIdAt(bx - bsize + x, by, bz - bsize + z) != newmats[x][z]) {
                        h.put(s.getBlockAt(bx - bsize + x, by, bz - bsize + z));
                    }
